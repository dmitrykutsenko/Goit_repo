...
Розробіть клас Contacts. Він повинен ініціалізувати через конструктор дві властивості: filename - 
ім'я файлу для пакування об'єкта класу Contacts, contacts - список контактів, екземплярів класу Person.
...
# Save the Contacts object to a file
persons.save_to_file()

# Read the Contacts object from the file
persons_from_file = Contacts("user_class.dat").read_from_file()

# Check equality
print(persons == persons_from_file)  # True
print(persons.contacts[0] == persons_from_file.contacts[0])  # True
print(persons.contacts[0].name == persons_from_file.contacts[0].name)  # True
print(persons.contacts[0].email == persons_from_file.contacts[0].email)  # True
print(persons.contacts[0].phone == persons_from_file.contacts[0].phone)  # True
